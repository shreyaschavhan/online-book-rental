...
require('includes/config.php');
	...
	{
		$msg=array();
		if(empty($_POST['owner']) || empty($_POST['name']) || empty($_POST['description']) || empty($_POST['publisher'])|| empty($_POST['edition']) || empty($_POST['isbn']) || empty($_POST['pages']) || empty($_POST['price']) || empty($_POST['rent']))
		{
		...
		{
			$msg[]="Rent must be in Numeric  Format...";
		}
		...
		else
		{
			...
			$query="insert into book(b_own,b_nm,b_subcat,b_desc,b_edition,b_publisher,b_isbn,b_page,b_price,b_rent,b_img,b_pdf)
			values('$b_own','$b_nm','$b_cat','$b_desc','$b_edition','$b_publisher','$b_isbn',$b_pages,$b_price,$b_rent,'$b_img','$b_pdf')";
